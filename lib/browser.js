(function() {
	...
	BROWSER.analyze = function(uaString) {
		...
		if (res !== null) {
			...
			setRetName(ret, rep);
		} /*else if (/Galaxy/i.test(ret.ua)
			&& !/Chrome/i.test(ret.ua)) {
			...
			ret.image = "opera-2";
		} else if (/Opera Labs/i.test(ret.ua)
			|| (/Opera/i.test(ret.ua)
				&& /Edition Labs/i.test(ret.ua))) {
						ret.full = getVersion(ret, 'Opera Labs')
			ret.image = "opera-next";
		} else if (/Opera Next/i.test(ret.ua)
			|| (/Opera/i.test(ret.ua)
				&& /Edition Next/i.test(ret.ua))) {
		...
		} else if (/Series60/i.test(ret.ua)
			&& !/Symbian/i.test(ret.ua)) {
						$title = "Nokia " . self::detect_browser_version(array('', 'Series60'));
			ret.image = "s60";
		} else if (/S60/i.test(ret.ua)
			&& !/Symbian/i.test(ret.ua)) {
						$title = "Nokia " . self::detect_browser_version(array('', 'S60'));
			ret.image = "s60";
		} else if (/SE\ /i.test(ret.ua)
			&& /MetaSr/i.test(ret.ua)) {
		...
		} else if (/AppleWebkit/i.test(ret.ua)
			&& /Android/i.test(ret.ua)
			&& !/Chrome/i.test(ret.ua)) {
			...
			} else if (preg_match('/\ rv:([.0-9a-zA-Z]+)/i', $useragent, $regmatch)) {
				// We have IE11 or newer
			}
			ret.image = "msie";
