	...
	var analyzeOther = function (ret) {
		...
		if (/Android|ADR /i.test(ret.ua)) {
			...
			if (rep = ret.ua.match(/(Mac OS ?X)/i)) {
				ret.name = substr($useragent, strpos(strtolower($useragent), strtolower($regmatch[1])));
				ret.name = substr($title, 0, strpos($title, ")"));

				if (strpos($title, ";")) {
					ret.name = substr($title, 0, strpos($title, ";"));
				}

				ret.name = str_replace("_", ".", $title);

				ret.image = $regmatch[1] == "Mac OSX" ? "mac-2" : "mac-3";
